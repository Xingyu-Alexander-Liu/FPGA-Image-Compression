...
// `define INPUT_FILE_NAME "../data/fractal1-0.d1e"
`define INPUT_FILE_NAME "../data/fractal1-0.d1e"
...
// `define VALIDATION_FILE_NAME "../data/fractal1-0.d1e"
// milestone 2 (post-DCT)
`define VALIDATION_FILE_NAME "../data/fractal1-0.d2e"
// encoded file
// `define VALIDATION_FILE_NAME "../data/motorcycle-0.mic"
...
// Task for filling the SRAM directly to shorten simulation time
	task fill_SRAM;
		...
		integer actual_sram_address, num_y, num_u; // 新增用于地址跳跃的变量
		logic [15:0] buffer;
	begin
		// pre-dct, 2 data per location
		num_y = (IMAGE_WIDTH * IMAGE_HEIGHT) / 2;
		num_u = (IMAGE_WIDTH * IMAGE_HEIGHT) / 4;
		...
		if (`MILESTONE == 1) begin
			$display("For milestone 1 we remove the header of PPM file");
		...
		while (file_data != `FEOF) begin
			...
			// follow the fixed yuv address offset for m2
			if (`MILESTONE == 2) begin
				if (i < num_y) begin
					actual_sram_address = 20'd614400 + i;
				end else if (i < num_y + num_u) begin
					actual_sram_address = 20'd768000 + (i - num_y);
				end else begin
					actual_sram_address = 20'd844800 + (i - num_y - num_u);
				end
			end else begin
				actual_sram_address = i + input_sram_offset;
			end

			SRAM_component.SRAM_data[actual_sram_address] = buffer;
			i++;
...
// Task for comparing the SRAM with the software-generated result
	task compare_result;
		...
		integer actual_compare_address, num_y, num_u;
	...
	begin
		...
		if (`MILESTONE == 1) begin
			...
			//2 pixel per location
			num_y = compare_size / 2;
			num_u = compare_size / 4;
		end else if (`MILESTONE == 2) begin
			...
			//1 s' per lcoation
			num_y = IMAGE_WIDTH * IMAGE_HEIGHT;
			num_u = (IMAGE_WIDTH * IMAGE_HEIGHT) / 2;
		end else if (`MILESTONE == 3) begin
			compare_sram_offset = 'd614400;
			// ************************************
		...
		while ((file_data != `FEOF) && (i < compare_size)) begin
			...
			if (`MILESTONE == 1) begin
				if (i < num_y) begin
					actual_compare_address = 20'd614400 + i;
				end else if (i < num_y + num_u) begin
					actual_compare_address = 20'd768000 + (i - num_y);
				end else begin
					actual_compare_address = 20'd844800 + (i - num_y - num_u);
				end
			end else if (`MILESTONE == 2) begin
				if (i < num_y) begin
					actual_compare_address = 20'd0 + i;
				end else if (i < num_y + num_u) begin
					actual_compare_address = 20'd307200 + (i - num_y);
				end else begin
					actual_compare_address = 20'd460800 + (i - num_y - num_u);
				end
			end else begin
				actual_compare_address = i + compare_sram_offset;
			end

			sram_data = SRAM_component.SRAM_data[actual_compare_address];

			if (sram_data != buffer) begin
				number_of_mismatches = number_of_mismatches + 1;
				$write("Mismatch #%3d at SRAM location %6d: %4h vs %4h\n",
					number_of_mismatches, actual_compare_address, sram_data, buffer);
