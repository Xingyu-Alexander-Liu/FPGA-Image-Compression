...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            S_MEGA_FS_CSP_CC_7: begin
                ...
                if(sram_fs_offset_flag == 2'd0) begin 
                    ...
                    if (sc_s == 5'd31) begin
                        if (cb_s == MAX_CB_Y) begin
                            ...
                            if (rb_s == MAX_RB) begin
                                rb_s <= 6'd0;             // 到达 Y 图像右下角，行归零
                                sram_fs_offset_flag <= 2'd1; // 完美时机：Y 的最后一个像素读完，切换到 U
                            end else begin
                                rb_s <= rb_s + 6'd1;      // 行增加
                            end
                        end else begin
                            cb_s <= cb_s + 7'd1;          // 没到行末，列增加
                        end
                    end
                    ...
                    if (sc_s == 5'd31) begin
                        if (cb_s == MAX_CB_UV) begin          // 注意：U 的列宽最大是 39
                            cb_s <= 7'd0;                 // 换行，列归零
                            if (rb_s == MAX_RB) begin
                                rb_s <= 6'd0;             // 到达 U 图像右下角，行归零
                                sram_fs_offset_flag <= 2'd2; // 完美时机：U 的最后一个像素读完，切换到 V
                            end else begin
                                rb_s <= rb_s + 6'd1;      // 行增加
                            end
                        end else begin
                    ...
                    if (sc_s == 5'd31) begin
                        if (cb_s == MAX_CB_UV) begin          // 注意：V 的列宽最大是 39
                            cb_s <= 7'd0;                 // 换行，列归零
                            if (rb_s == MAX_RB) begin
                                rb_s <= 6'd0;             // 到达 V 图像右下角，行归零
                                sram_fs_offset_flag <= 2'd0; // 整个图像读取完毕，重置 flag
                                finish_mega_flag <= 1'b1;         // 发出完成信号
                            end else begin
                                rb_s <= rb_s + 6'd1;      // 行增加
                            end
                        end else begin
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            S_MEGA_WSP_CT_LO_4: begin
                ...
                if(sram_wsp_offset_flag == 2'd0) begin 
                    ...
                    if (sc_sp_sram == 6'd63) begin
                        if (cb_sp == MAX_CB_Y) begin
                            ...
                            if (rb_sp == MAX_RB) begin
                                rb_sp <= 6'd0;             // 到达 Y 图像右下角，行归零
                                sram_wsp_offset_flag <= 2'd1; // 完美时机：Y 的最后一个像素读完，切换到 U
                            end else begin
                                rb_sp <= rb_sp + 6'd1;      // 行增加
                            end
                        end else begin
                            cb_sp <= cb_sp + 7'd1;          // 没到行末，列增加
                        end
                    end
                    ...
                    // FSM is fetching U value from SRAM
                    if (sc_sp_sram == 6'd63) begin
                        if (cb_sp == MAX_CB_UV) begin          // 注意：U 的列宽最大是 39
                            cb_sp <= 7'd0;                 // 换行，列归零
                            if (rb_sp == MAX_RB) begin
                                rb_sp <= 6'd0;             // 到达 U 图像右下角，行归零
                                sram_wsp_offset_flag <= 2'd2; // 完美时机：U 的最后一个像素读完，切换到 V
                            end else begin
                                rb_sp <= rb_sp + 6'd1;      // 行增加
                            end
                        end else begin
                    ...
                    // FSM is fetching V value from SRAM
                    if (sc_sp_sram == 6'd63) begin
                        if (cb_sp == MAX_CB_UV) begin          // 注意：V 的列宽最大是 39
                            cb_sp <= 7'd0;                 // 换行，列归零
                            if (rb_sp == MAX_RB) begin
                                rb_sp <= 6'd0;             // 到达 V 图像右下角，行归零
                                sram_wsp_offset_flag <= 2'd0; // 整个图像读取完毕，重置 flag
                                //finish_mega_flag <= 1'b1;         // 发出完成信号
                            end else begin
                                rb_sp <= rb_sp + 6'd1;      // 行增加
                            end
...
/*
            ...
            S_CT_LI_2: begin
                s0_reg_0 <= read_data_a[0][7:0];
                s1_reg_0 <= read_data_a[0][15:8];
                s2_reg_0 <= read_data_a[0][23:16];
                s3_reg_0 <= read_data_a[0][31:24];
                m2_state <= S_CT_LI_3;
            end
            S_CT_LI_3: begin
                select0 <= 5'd1;
                ...
                c_idx[2] <= 5'd2;
                c_idx[3] <= 5'd3;
                s4_reg_0 <= read_data_a[0][7:0];
                s5_reg_0 <= read_data_a[0][15:8];
                s6_reg_0 <= read_data_a[0][23:16];
                s7_reg_0 <= read_data_a[0][31:24];
                m2_state <= S_CT_LI_4;                
            end
            S_CT_LI_4: begin
                select0 <= 5'd2;
...
if (sc_s == 5'd31) begin
    if (cb_s == 7'd79) begin
        cb_s <= 7'd0;                 // 换行，列归零
        rb_s <= (rb_s == 6'd59) ? 6'd0 : (rb_s + 6'd1); // 行增加，若到最后一行则归零
    end else begin
        cb_s <= cb_s + 7'd1;          // 没到行末，列增加
        // rb_s 保持不变 (触发器默认行为，不用写)
    end
end
...
if(sram_fs_offset_flag == 2'd0) begin 
    ...
    // FSM is fetching U value from SRAM
    if (sc_s == 5'd31) begin
        if (cb_s == 7'd39) begin          // 注意：U 的列宽最大是 39
            cb_s <= 7'd0;                 // 换行，列归零
            if (rb_s == 6'd59) begin
                rb_s <= 6'd0;             // 到达 U 图像右下角，行归零
                sram_fs_offset_flag <= 2'd2; // 完美时机：U 的最后一个像素读完，切换到 V
    ...
    // FSM is fetching V value from SRAM
    if (sc_s == 5'd31) begin
        if (cb_s == 7'd39) begin          // 注意：V 的列宽最大是 39
            cb_s <= 7'd0;                 // 换行，列归零
            if (rb_s == 6'd59) begin
                rb_s <= 6'd0;             // 到达 V 图像右下角，行归零
                sram_fs_offset_flag <= 2'd0; // 整个图像读取完毕，重置 flag
...
*/
/*
write s' sram management
                if(sram_wsp_offset_flag == 2'd0) begin 
                    // FSM is fetching Y value from SRAM
                    if (sc_sp_sram == 6'd63) begin
                        if (cb_sp == 7'd79) begin
                            cb_sp <= 7'd0;                 // 换行，列归零
                            if (rb_sp == 6'd59) begin
                                rb_sp <= 6'd0;             // 到达 Y 图像右下角，行归零
                                sram_wsp_offset_flag <= 2'd1; // 完美时机：Y 的最后一个像素读完，切换到 U
                            end else begin
                                rb_sp <= rb_sp + 6'd1;      // 行增加
                    ...
                    // FSM is fetching U value from SRAM
                    if (sc_sp_sram == 6'd63) begin
                        if (cb_sp == 7'd39) begin          // 注意：U 的列宽最大是 39
                            cb_sp <= 7'd0;                 // 换行，列归零
                            if (rb_sp == 6'd59) begin
                                rb_sp <= 6'd0;             // 到达 U 图像右下角，行归零
                                sram_wsp_offset_flag <= 2'd2; // 完美时机：U 的最后一个像素读完，切换到 V
                    ...
                    // FSM is fetching V value from SRAM
                    if (sc_sp_sram == 6'd63) begin
                        if (cb_sp == 7'd39) begin          // 注意：V 的列宽最大是 39
                            cb_sp <= 7'd0;                 // 换行，列归零
                            if (rb_sp == 6'd59) begin
                                rb_sp <= 6'd0;             // 到达 V 图像右下角，行归零
                                sram_wsp_offset_flag <= 2'd0; // 整个图像读取完毕，重置 flag
